...
export default function UpdateBlogPage({ close, updatedBlogData }: TBlogProps) {
  ...
  //updated blog
  useEffect(() => {
    setTitle(updatedBlogData.title);
    setAuthor(updatedBlogData.author);
    setyear(updatedBlogData.year);
    setSummary(updatedBlogData.summary);
    setContent(updatedBlogData.content);
    setThumbnailPreview(updatedBlogData.img);
  }, [updatedBlogData]);
  ...
  return (
    <div className="px-4 md:px-7">
      <div className="flex justify-end">
        <p onClick={() => close()}>
          {" "}
          <X className="text-lg hover:text-red-700 cursor-pointer "></X>{" "}
        </p>
      </div>
